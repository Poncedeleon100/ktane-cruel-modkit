...
{
    List<int> bulb1Notes = new List<int>();
    List<int> bulb2Notes = new List<int>();
    bool[] bulbStates = new bool[2];
    bool[] bulbSolved = new bool[2];
    int lastPress = -1;
    ...
    public AVInput(CruelModkitScript Module, int ModuleID, ComponentInfo Info, bool Vanilla, byte Components) : base(Module, ModuleID, Info, Vanilla, Components)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            if (bulb1Actions[i] != 2) bulb1Actions[i] = Random.Range(0, 2);
            if (bulb2Actions[i] != 2) bulb2Actions[i] = Random.Range(0, 2);
        }
    ...
    public override void OnPianoPress(int Piano)
    {
        ...
        if (BulbScrewedIn[0] & BulbScrewedIn[1])
        {
            if (!bulbSolved[0])
            {
                if (bulb1Notes.Contains(Piano)) ChangeBulb(0, !bulbStates[0]);
                else ChangeBulb(0, bulb1Actions[Piano] == 1);
            }
            if (!bulbSolved[1])
            {
                if (bulb2Notes.Contains(Piano)) ChangeBulb(1, !bulbStates[1]);
                else ChangeBulb(1, bulb2Actions[Piano] == 1);
            }
        }
        else
        {
    ...
    public override void OnBulbButtonPress(int Button)
    {
        ...
        else
        {
            ...
            {
                Debug.LogFormat("[The Cruel Modkit #{0}] Strike! Pressed the I key for resetting after the black key {1}.", ModuleID, Info.PianoKeyNames[lastPress]);
                Module.CauseStrike();
                return;
            }
        }
