...
public class SimonSkips : Puzzle
{
    ...
    public SimonSkips(CruelModkitScript Module, int ModuleID, ComponentInfo Info, bool Vanilla, byte Components) : base(Module, ModuleID, Info, Vanilla, Components)
    {
        ...
        {
            Debug.LogFormat("[The Cruel Modkit #{0}] The starting colour is white or black. Submit an empty sequence!", ModuleID);
            submitEmpty = true;
        }
    }
    ...
    public override void OnUtilityPress()
    {
        ...
        if (Module.IsModuleSolved() || Module.Solving)
            return;
    ...
    public override void OnArrowPress(int Arrow)
    {
        ...
        if (Arrow == 8)
        {
            ...
            {
                Debug.LogFormat("[The Cruel Modkit #{0}] The correct sequence has been entered. Solve!", ModuleID);
                Module.Solve();
            }
            else
            {
                string[] inputtedColours = inputtedSequence.Where(x => x != 8).Select(x => ComponentInfo.ArrowColors[orderedArrows[x]]).ToArray();
                Debug.LogFormat("[The Cruel Modkit #{0}] An incorrect sequence of {1} has been submitted. Strike!", ModuleID, string.Join(", ", inputtedColours));
                Module.CauseStrike();
