...
public class StumblingSymphony : Puzzle
{
    ...
    List<List<int>> noteIndices = new List<List<int>>();

    int activeRule;

    List<int> finalInput = new List<int>();
    List<int> buttonPresses = new List<int>(); // After which note input should you press the button
    int numInputs;
    bool buttonPressed;

    int semiShift;

    public StumblingSymphony(CruelModkitScript Module, int ModuleID, ComponentInfo Info, bool Vanilla, byte Components) : base(Module, ModuleID, Info, Vanilla, Components)
    {
