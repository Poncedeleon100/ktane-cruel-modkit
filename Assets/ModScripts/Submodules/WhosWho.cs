...
public class WhosWho : Puzzle {
  ...
  Stopwatch O_PressTime = new Stopwatch();
  Stopwatch I_PressTime = new Stopwatch();
  ...
  public WhosWho(CruelModkitScript Module, int ModuleID, ComponentInfo Info, bool Vanilla, byte Components) : base(Module, ModuleID, Info, Vanilla, Components) {
  ...
  public override void OnBulbButtonRelease(int Button) {
    ...
    if (O_PressTime.Elapsed.TotalSeconds >= 1 || I_PressTime.Elapsed.TotalSeconds >= 1) {
      if (!submissionMode) {
        ...
        UnityEngine.Debug.LogFormat("[The Cruel Modkit #{0}] Correctly submitted {1}, solved!", ModuleID, finalNumber);
        Module.StartSolve();
        Module.Solve();
      } else {
